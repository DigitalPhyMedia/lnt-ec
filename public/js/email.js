...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function checkFormValidity() {
        ...
        // Email is optional now, so we only validate it if it's not empty
    ...
    form.addEventListener('submit', function(event) {
        ...
        // Initialize emailParams with required fields
        let emailParams = {
            name: name,
            mobile: mobile,
        ...
        if (email !== '') {
            // When email is provided, use this template and include email in params
            templateId = "template_zejs2ig";
            emailParams.email = email;
        } else {
            // When no email is provided, use the template for name and phone only
            templateId = "template_2vd795r";
        }
        
        emailjs.send("service_8vamjmf", templateId, emailParams)
